#!/usr/bin/bash
# Usage: ./msg.sh <recipient_name> <message>
#
# Abstractisation which also sends dh point if needed and fetches new dh points from that person
...
if [ ! -d "$PEOPLE_DIR/$DANGER_USER_other_person" ]; then
    die "Error: User '$DANGER_USER_other_person' does not exist. Please add them first using ./add_person.sh"
fi
other_person="$DANGER_USER_other_person"

# If there is not point for that person, send one
if [ -z "$(ls -A "$PEOPLE_DIR/$other_person/$DH_MY_POINTS" 2>/dev/null)" ]; then
...
if [ -z "$(ls -A "$PEOPLE_DIR/$other_person/$DH_RECEIVED_POINTS" 2>/dev/null)" ]; then
